...
use crate::types::{
    BuildCommonResult, BuildListOptions, BuildLogsResult, BuildResult, BuildStageResult,
    BuildStatusResult, StartBuildRequest,
};
use std::fmt::Write;
...
impl CnbClient {
    ...
    pub async fn get_build_logs(
        ...
        if let Some(ref event) = opts.event {
            let _ = write!(url, "&event={}", encode(event));
        }
        if let Some(ref source_ref) = opts.source_ref {
            let _ = write!(url, "&sourceRef={}", encode(source_ref));
        }
        if let Some(ref user_name) = opts.user_name {
            let _ = write!(url, "&userName={}", encode(user_name));
        }
        if let Some(ref create_time) = opts.create_time {
            let _ = write!(url, "&createTime={}", encode(create_time));
        }
        if let Some(ref end_time) = opts.end_time {
            let _ = write!(url, "&endTime={}", encode(end_time));
        }
        let resp = self.send_with_retry(|| self.http.get(&url)).await?;
        Self::handle_response(resp).await
    }
