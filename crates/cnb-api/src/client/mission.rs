...
impl CnbClient {
    ...
    pub async fn list_missions(
        &self, slug: &str, search: Option<&str>, order_by: Option<&str>, desc: bool,
        page: u32, page_size: u32,
    ) -> Result<Vec<Mission>, ApiError> {
        let slug = Self::encode_path(slug);
        let mut url = format!("{}{slug}/-/missions?page={page}&page_size={page_size}", self.base_url);
    ...
    /// 创建任务集
    pub async fn create_mission(&self, slug: &str, req: &CreateMissionRequest) -> Result<(), ApiError> {
        let slug = Self::encode_path(slug);
    ...
    /// 设置任务集可见性
    pub async fn set_mission_visibility(&self, mission: &str, req: &SetMissionVisibilityRequest) -> Result<(), ApiError> {
        let mission = Self::encode_path(mission);
    ...
    /// 获取视图配置
    pub async fn get_mission_view_config(&self, mission: &str, view_id: &str) -> Result<MissionViewConfig, ApiError> {
        let mission = Self::encode_path(mission);
        let url = format!("{}{mission}/-/mission/view?id={}", self.base_url, encode(view_id));
        let resp = self.send_with_retry(|| self.http.get(&url)).await?;
        Self::handle_response(resp).await
    }

    /// 设置视图配置
    pub async fn set_mission_view_config(&self, mission: &str, config: &MissionViewConfig) -> Result<(), ApiError> {
        let mission = Self::encode_path(mission);
    ...
    /// 添加/修改视图
    pub async fn put_mission_view(&self, mission: &str, views: &[MissionView]) -> Result<(), ApiError> {
        let mission = Self::encode_path(mission);
        let url = format!("{}{mission}/-/mission/view-list", self.base_url);
    ...
    /// 排序视图
    pub async fn sort_mission_views(&self, mission: &str, views: &[MissionView]) -> Result<(), ApiError> {
        let mission = Self::encode_path(mission);
        let url = format!("{}{mission}/-/mission/view-list", self.base_url);
