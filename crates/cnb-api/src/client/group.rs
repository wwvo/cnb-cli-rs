...
impl CnbClient {
    ...
    pub async fn list_groups_under(
        &self, slug: &str, opts: &ListGroupsOptions,
    ) -> Result<Vec<Group>, ApiError> {
        let slug = Self::encode_path(slug);
        let url = format!(
            "{}user/groups/{}?{}",
            self.base_url, slug, opts.query_string()
        );
        let resp = self.send_with_retry(|| self.http.get(&url)).await?;
        Self::handle_response(resp).await
    }
    ...
    pub async fn update_group(
        &self, group: &str, req: &UpdateGroupRequest,
    ) -> Result<(), ApiError> {
        let group = Self::encode_path(group);
        let url = format!("{}{}", self.base_url, group);
    ...
    pub async fn transfer_group(
        &self, group: &str, req: &TransferGroupRequest,
    ) -> Result<(), ApiError> {
        let group = Self::encode_path(group);
    ...
    pub async fn list_subgroups(
        &self, slug: &str, opts: &ListSubGroupsOptions,
    ) -> Result<Vec<Group>, ApiError> {
        let slug = Self::encode_path(slug);
        let url = format!(
            "{}{}/-/sub-groups?{}",
    ...
    pub async fn update_group_setting(
        &self, slug: &str, req: &UpdateGroupSettingRequest,
    ) -> Result<(), ApiError> {
        let slug = Self::encode_path(slug);
        let url = format!("{}{}/-/settings", self.base_url, slug);
        let resp = self.http.put(&url).json(req).send().await?;
    ...
    /// 获取组织特权额度
    pub async fn get_special_amount(
        &self, slug: &str,
    ) -> Result<SpecialAmount, ApiError> {
        let slug = Self::encode_path(slug);
    ...
    pub async fn list_group_members(
        &self, group: &str, opts: &ListGroupMembersOptions,
    ) -> Result<Vec<GroupMember>, ApiError> {
        let group = Self::encode_path(group);
        let url = format!(
            "{}{}/-/members?{}",
            self.base_url, group, opts.query_string()
        );
        let resp = self.send_with_retry(|| self.http.get(&url)).await?;
        Self::handle_response(resp).await
    }
    ...
    pub async fn add_group_member(
        &self, group: &str, username: &str, req: &GroupMemberRequest,
    ) -> Result<(), ApiError> {
        let group = Self::encode_path(group);
        let url = format!(
            "{}{}/-/members/{}",
            self.base_url, group, urlencoding::encode(username)
        );
        let resp = self.http.post(&url).json(req).send().await?;
        Self::handle_empty_response(resp).await
    }
