...
2. 对 `x86_64-unknown-linux-gnu`，除 `.tar.gz` 外，还会额外生成 Linux 原生包：`.deb` 和 `.rpm`
3. 生成与 CNB 一致的 `LATEST_CHANGELOG.md`
4. 基于最终 release 附件生成 `sha256sum.txt`，其中包含 `.tar.gz`、`.zip`、`.deb`、`.rpm`
5. 上传附件和 `sha256sum.txt` 到 GitHub Release
6. 先删除 CNB 对应 Release 的旧附件，再回填同一批新文件
...
- 首轮仅对 `x86_64-unknown-linux-gnu` 发布 `.deb` / `.rpm`
